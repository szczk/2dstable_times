...
#PBS -N 2descape_a_${alpha}_s_${sigma}_${num}
...
# ./generator.x --alpha ${alpha} --noise ${sigma} --storage "${output}"  --tmp "${tmp}" --prefix "escape" --data_file_num ${num}  > $logs/escape_2d_a${alpha}_s${sigma}_n${num}.log
./analysis.x --alpha ${alpha} --noise ${sigma} --storage "$STORAGE/escape/" --data "$SCRATCH/escape/"  --tmp "$SCRATCH" --prefix "dt04" --data_file_num 5 >> $STORAGE/logs/2descape_analysis_a${alpha}_s${sigma}_pi20.log
